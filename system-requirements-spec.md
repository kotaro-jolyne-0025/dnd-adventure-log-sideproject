...
### 1.2 系統定位
本系統為**有結構的數位記錄工具**，所有數值欄位由玩家手動填寫，系統不做自動計算或自動帶入。
...
| 職業/等級（動態列）| 陣列，每筆含職業名稱＋等級數字 | ✅ 至少一筆 | 例：聖騎士 Lv.6、術師 Lv.4 |
| 派系 | 文字（最多 100 字）| ❌ | 選填 |

#### 2.1.2 功能清單
| 功能 | 說明 |
|---|---|
| 建立角色 | 填寫表單新增角色，可「開卡」但不新增冒險記錄 |
| 查看角色列表 | 卡片形式顯示所有角色 |
| 編輯角色 | 修改角色資料（含動態職業/等級列）|
...
| 起始金幣 | 數字（小數點後 2 位）| ❌ | 手動填寫 |
| 金幣變化 | 數字（小數點後 2 位）| ❌ | 正負值均可 |
| 金幣合計 | 數字（小數點後 2 位）| ❌ | 手動填寫 |
| 起始休假天數 | 整數 | ❌ | 手動填寫 |
| 休假天數變化 | 整數 | ❌ | 正負值均可 |
| 休假天數合計 | 整數 | ❌ | 手動填寫 |
| 起始魔法物品數 | 整數 | ❌ | 手動填寫 |
