...
| 功能 | 說明 |
|---|---|
| 新增冒險記錄 | 填寫表單，起始值自動帶入，合計由系統計算；採用獨立資源卡片化（金幣/休整期/魔法物品）與即時合計徽章；支援記錄獲得的永久魔法物品與消耗品（可自訂數量），儲存時自動同步寫入倉庫 (Inventory) |
| 查看冒險紀錄表 | 時間軸卡片形式，依遊玩日期由舊到新排序，展示等級推進與各項數值 Delta 變動標籤 |
| 查看冒險記錄詳情 | 分區塊顯示（基本資訊 / 資源變動 / 備註 / 獲得永久性魔法物品 / 獲得消耗品 / 休整期活動）|
| 編輯冒險記錄 | 修改記錄資料；為保護已在倉庫中進行的道具消耗或數量異動，編輯冒險記錄時不覆寫倉庫現有庫存（庫存由倉庫模組獨立管理）|
| 刪除冒險記錄 | 刪除單筆記錄，自動觸發全域廣播並即時重算頂部看板資源 |
